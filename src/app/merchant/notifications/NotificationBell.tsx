    ...
    return (
        <div className="relative" ref={dropdownRef}>
            <button 
                ...
                className="relative w-10 h-10 flex items-center justify-center rounded-xl bg-slate-50 text-slate-500 hover:bg-slate-100 transition-all border border-slate-200"
            >
            ...
            {isOpen && (
                <div className="absolute left-0 mt-3 w-80 bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden z-[100] animate-in fade-in slide-in-from-top-2 duration-200">
                    <div className="p-4 border-b border-slate-100 flex items-center justify-between">
                        <h3 className="font-black text-sm text-slate-800">الإشعارات</h3>
                        {unreadCount > 0 && (
                            <button 
                            ...
                            <div className="divide-y divide-slate-50">
                                {notifications.map((n) => (
                                    ...
                                    >
                                        ...
                                        <h4 className={`text-xs ${!n.is_read ? 'font-black text-slate-900' : 'font-bold text-slate-600'}`}>{n.title}</h4>

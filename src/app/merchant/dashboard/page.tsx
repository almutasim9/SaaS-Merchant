...
export default function MerchantDashboard() {
    ...
    const fetchDashboardData = async (userId: string) => {
        ...
        try {
            ...
            const [allOrdersResult, productsResult, ordersResult, sectionsResult, totalSalesOrders, pendingOrdersResult] = await Promise.all([
                ...
                supabase.from('orders').select('total_price, delivery_fee').eq('store_id', activeStore.id).in('status', ['completed', 'delivered']),
                supabase.from('orders').select('id', { count: 'exact', head: true }).eq('store_id', activeStore.id).eq('status', 'pending')
            ]);
            ...
            if (allOrders) {
                // Determine overarching numbers instead of just today
                const allCompleted = totalSalesOrders.data || [];
                const totalSales = allCompleted.reduce((acc, order) => acc + ((order.total_price || 0) - (order.delivery_fee || 0)), 0);

                const newOrdersCount = pendingOrdersResult.count || 0;
                const avg = allCompleted.length > 0 ? totalSales / allCompleted.length : 0;
